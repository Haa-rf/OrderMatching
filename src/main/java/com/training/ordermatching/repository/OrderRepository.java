...
public interface OrderRepository extends JpaRepository<Order, Long> {
    ...
    @Query(value = "select * from orders where side='BUY' and status='pending' order by price desc LIMIT 20", nativeQuery = true)
    List<Order> findPendingBuyOrderLimit20();

    @Query(value = "select * from orders where side='SELL' and status='pending' order by price asc LIMIT 20", nativeQuery = true)
